...
kpi_row([
    ...
    {"label": "最新交易日", "value": (stats.get("last_datetime") or "—")[:10],
     "tone": "neutral"},
    {"label": "仓库根", "value": str(stats.get("root", ""))[:18] + ("…" if len(str(stats.get("root", ""))) > 18 else ""),
     "tone": "neutral"},
])
...
with c2:
    st.write("")
    st.write("")
    _warm_btn = st.button("🔥 预热", type="primary", width="stretch")
...
if _warm_btn:
    ...
    if not syms:
        ...
        if guard_error(res, "预热"):
            st.stop()
        st.success(f"预热完成: 成功 {res.get('warmed', 0)} / 失败 {res.get('failed', 0)}")
        for r in res.get("results") or []:
            _tag = badge(f"{r.get('n', 0)} 根 · 源 {r.get('source', '?')}", "success") \
                if r.get("ok") else badge(f"失败: {r.get('error', '?')[:40]}", "danger")
            st.markdown(f"- **{r.get('symbol')}** {_tag}", unsafe_allow_html=True)
        st.session_state.qm_cache_stats = _load_stats()
        st.rerun()
